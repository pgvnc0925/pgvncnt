...
import { ContextFrame, StepCard, Verdict, TensionMap } from '@/components/blocks';
import { Input, Select, Scale, Textarea } from '@/components/atomic';
import { useAppConfig, useAppSession, useCredits, useSubmitStep } from '@/hooks';
import { InputField, Step, StructuredDossier } from '@/lib/app-types';
import { n8nClient } from '@/lib/api/n8n-client';
import { ExitToLandingButton } from '@/components/app/exit-to-landing-button';
import coreAppUISchema from '@/apps/core-app-v1/ui-schema';
...
export default function CoreAppPage() {
  ...
  // State locale - TUTTI gli useState devono essere qui in cima
  ...
  const stepIndex = useMemo(() => {
    return coreAppUISchema.steps.findIndex((s) => s.id === currentStepId);
  }, [currentStepId]);

  // Update stepResponse when currentStepId changes
  useEffect(() => {
    const currentResponse = responses[currentStepId];
    if (currentResponse && typeof currentResponse === 'object') {
      setStepResponse(currentResponse as Record<string, unknown>);
    } else {
      setStepResponse({});
    }
  }, [currentStepId, responses]);
    ...
    try {
      ...
      const dossier: StructuredDossier = {
        ...
        context: {
          ...
          market: (responses.marketGeography as 'locale' | 'regionale' | 'nazionale' | 'internazionale') || 'locale',
        ...
        diagnosticFlags: {
          valueClarity: responses.differentiation as number,
          positioningMaturity: (responses.differentiation as number) < 50 ? 'early' : 'advanced',
          conversionRate: responses.conversionRate as number,
          retention: responses.clientRetention as string,
        },
        ...
        >
          <div className="space-y-12">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 text-center space-y-4">
              ...
              <button className="pv-button-base bg-primary text-white px-6 py-3 rounded-lg">
          ...
          <div className="space-y-8">
            ...
            <div className="flex gap-4">
              <button
                ...
                className="pv-button-base border border-slate-300 text-slate-900 px-6 py-3 disabled:opacity-50 rounded-lg"
              >
                ← Indietro
              </button>
              <button
                ...
                className="pv-button-base bg-primary text-white px-6 py-3 flex-1 disabled:opacity-50 rounded-lg"
              >
