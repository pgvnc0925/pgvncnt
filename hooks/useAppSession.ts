...
import { AppSession, StepResponse, StructuredDossier } from '@/lib/app-types';
import * as supabase from '@/lib/api/app-supabase';
...
export function useAppSession(appId: string, userId: string) {
  ...
  const completeSession = useCallback(
    async (verdict: string, tensionMap: Record<string, unknown>, dossier: StructuredDossier | Record<string, unknown>) => {
      if (!session) return;
