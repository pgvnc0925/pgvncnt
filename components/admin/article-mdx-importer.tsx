...
interface ArticleMdxImporterProps {
  onImport: (data: ArticleFormData) => void;
}

export function ArticleMdxImporter({ onImport }: ArticleMdxImporterProps) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      onImport({
        ...
        ctaBookSlug: frontmatter.ctaBookSlug || "",
        ctaBookText: frontmatter.ctaBookText || "",
        affiliateLink: frontmatter.amazonAffiliateLink || frontmatter.amazonLink || "",
        faq: faqItems,
      });
  ...
  return (
    <Card className="border-2 border-dashed border-primary/30 bg-primary/5">
      <CardHeader>
        ...
        <CardDescription>
          Carica un file .mdx con frontmatter per auto-compilare il form articolo
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="flex items-center gap-4">
          <div className="flex-grow">
            <label htmlFor="article-mdx-import" className="cursor-pointer">
        ...
        <div className="p-3 bg-muted/50 rounded-lg">
          <p className="text-xs text-muted-foreground">
            <strong>Come funziona:</strong> Carica un file .mdx con frontmatter valido e i campi
            sottostanti verranno compilati automaticamente. Potrai modificarli prima di salvare.
          </p>
        </div>
      </CardContent>
    </Card>
